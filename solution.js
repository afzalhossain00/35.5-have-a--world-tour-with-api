...
const object = {
    yourName: 'Afzal',
    phoneNumber: '01912507592',
    carList: ['Toyota', 'Hundai', 'TATA', 'HINO'],
    assets: {
        cash: '1300 TK',
        bank: '20400 TK'
    },
    allInfo: function () {
        return (`Here is ${this.yourName}, His Phone Number is ${this.phoneNumber}. He has ${this.carList[1]} & His Bank balance is ${this.assets.bank}`)
    }
}
const output = object.allInfo();
console.log(output);
...
const onePara = (something) => something / 17;
console.log(onePara(50));
...
const twoPara = (one, two) => (one + two) / 2;
...
const threePara = (one, two) => {
    const first = one + 7;
    const second = two + 7;
    const final = first + second;
    return final;
}
const result = threePara(2, 2);
console.log(result);
...
const numbers = [3, 12, 5, 33, 575, 6, 77, 82];
...
const simpleObj = { name: 'Afzal', roll: 1193, class: 'graduate' };
const { roll } = simpleObj
console.log(roll);


const arrayD = [4, 5, 2, 7];
const [a, b, ...c] = arrayD;
const balance = b;
console.log(balance);
